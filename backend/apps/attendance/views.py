...
class TimeLogViewSet(viewsets.ModelViewSet):
    ...
    def clock_out(self, request):
        ...
        # Check if employee has a scheduled shift that allows clock-out
        from apps.scheduling.models import Shift
        eligible_shift = Shift.get_clockout_eligible_shift(employee)
        if not eligible_shift:
            return Response(
                {
                    'detail': 'No active scheduled shift found for clock-out. You can only clock out during an active scheduled shift.',
                    'requires_shift': True,
                    'current_time': timezone.now().isoformat(),
                },
                status=status.HTTP_403_FORBIDDEN
    ...
    def shift_status(self, request):
        ...
        clockout_eligible_shift = Shift.get_clockout_eligible_shift(employee)
        ...
        data = {
            ...
            'can_clock_out': bool(clockout_eligible_shift and active_log),
    ...
    def export(self, request):
        """Export employee time logs as CSV"""
        import csv
        ...
        if employee_id:
            try:
                ...
                filename = f"{employee.user.first_name}_{employee.user.last_name}_time_logs"
            except Employee.DoesNotExist:
                filename = f"employee_{employee_id}_time_logs"
        else:
            filename = "all_employees_time_logs"
        ...
        writer.writerow([
            'Employee ID', 'Employee Name', 'Email', 'Role',
            'Date', 'Clock In Time', 'Clock Out Time', 'Duration (Hours)',
            'Clock In Method', 'Status', 'Clock In Location', 'Clock Out Location', 'Notes'
        ])

        for log in queryset.order_by('-clock_in_time'):
            writer.writerow([
                log.employee.employee_id,
                f"{log.employee.user.first_name} {log.employee.user.last_name}",
                log.employee.user.email,
                log.employee.role.name if log.employee.role else '',
                log.clock_in_time.strftime('%Y-%m-%d') if log.clock_in_time else '',
                log.clock_in_time.strftime('%H:%M:%S') if log.clock_in_time else '',
                log.clock_out_time.strftime('%H:%M:%S') if log.clock_out_time else '',
                f"{log.duration_hours:.2f}" if log.duration_hours else '',
                log.clock_in_method or '',
                log.status,
                log.clock_in_location.name if log.clock_in_location else '',
                log.clock_out_location.name if log.clock_out_location else '',
                log.notes or ''
            ])

        logger.info(f"Time logs exported by admin user {request.user.username}")
        return response
